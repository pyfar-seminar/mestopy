...
def deconv(x_meas, x_ref):
    ...
    if x_meas.sampling_rate != x_ref.sampling_rate:
        raise ValueError("The two signals for deconvolution have different sampling rates!")
    ...
    # Check if both signals have the same length, if not: bring them to the same length
    if x_meas.n_samples > x_ref.n_samples:
        # Add Zeros to x_ref
            x_ref = Signal(np.concatenate((x_ref.time,
                    np.zeros(x_meas.n_samples - x_ref.n_samples))),
                    x_ref.sampling_rate,
                    fft_norm=x_ref.fft_norm,
                    dtype=x_ref.dtype,
                    comment=x_ref.comment)

    if x_meas.n_samples < x_ref.n_samples:
        # Add Zeros to x_meas
            x_meas = Signal(np.concatenate((x_meas.time,
                    np.zeros(x_ref.n_samples - x_meas.n_samples))),
    ...
    # Check if the signals have any comments, if yes: concatenate the comments for the result
    if not (x_meas.comment==None and x_ref.comment==None):
        H.comment = 'IR calculated with deconvolution: [1]' + x_meas.comment + '; [2]' +  x_ref.comment
    else:
...
class RefObj(object, Signal):
    ...
    def invert(self):
        ...
        self._ref_data = Signal(new_data, self.ref_data.sampling_rate(),
...
class MeasurementChain(object, Signal, RefObj):
    ...
    def rm_ref(self, num):
        ...
        # remove ref-object in chain by name
            ...
            raise TypeError("ref-object to remove must be named by int or str!")
